...
CREATE TABLE IF NOT EXISTS `boleteria_progra`.`seccion` (
  ...
  `precio_seccion` VARCHAR(45) NULL,
...
CREATE TABLE IF NOT EXISTS `boleteria_progra`.`asiento_reservado` (
  ...
  INDEX `fk_reservado_boleto_idx` (`id_boleto` ASC),
  ...
  CONSTRAINT `fk_reservado_evento`
    ...
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_reservado_boleto`
    FOREIGN KEY (`id_boleto`)
    REFERENCES `boleteria_progra`.`boleto` (`id_boleto`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
